...
export class RedisCacheService {
  ...
  async debugCache(pattern: string): Promise<void> {
    try {
      ...
      for (const key of keys) {
        ...
        this.logger.debug(`Key: ${key}, TTL: ${ttl}s, Size: ${value ? value.length : 0} bytes`);
      }
    } catch (error) {
      this.logger.error(`Error debugging cache with pattern ${pattern}: ${error.message}`);
    }
  }
  ...
  async set(key: string, value: any, ttlSeconds = 300): Promise<void> {
    try {
      ...
      this.logger.debug(`Cache SET for key: ${key}, TTL: ${ttlSeconds}s, Size: ${serialized.length} bytes`);
    } catch (error) {
   ...
   */
  generateSimpleKey(prefix: string, ...parts: (string | number | boolean | null | undefined)[]): string {
