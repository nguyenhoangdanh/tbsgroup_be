import {
  ...
  export class BagProcessHttpController {
    ...
    constructor(
      @Inject(BAG_PROCESS_SERVICE) private readonly bagProcessService: IBagProcessService,
    ) {}
  ...
  async listBagProcessesByIds(@Body('ids') ids: string[]) {
    ...
    const bagProcesses = await Promise.all(
      ids.map((id) => this.bagProcessService.getBagProcess(id).catch(() => null)),
    );
  ...
  async getProcessesByType(@Param('type') processType: string) {
    try {
      ...
      throw AppError.from(
        new Error(`Lỗi khi lấy danh sách công đoạn theo loại: ${error.message}`),
        400,
      );
    }
  }
