...
import { AppError, Paginated, ReqWithRequester } from 'src/share';
...
export abstract class BaseCrudController<T, C, U, F> {
  ...
  async list(@Query() filter: F, @Query('page') page = 1, @Query('limit') limit = 10, 
             @Query('sort') sort = 'createdAt', @Query('order') order = 'desc',
    ...
    try {
      ...
      const validOrder = (order && ['asc', 'desc'].includes(String(order).toLowerCase())) 
      ...
      const result = await this.service.listEntities(req.requester, filter, pagination);
      ...
      this.logger.error(`Error listing entities: ${error.message}`, error.stack);
      if (error instanceof AppError) {
        throw error;
      }
      throw AppError.from(error, HttpStatus.BAD_REQUEST);
  ...
  @Patch(':id')
  @HttpCode(HttpStatus.OK)
  async update(@Param('id') id: string, @Body() dto: U, @Request() req: ReqWithRequester) {
...
export function createCrudController<T, C, U, F = any>(options: {
  ...
  class CrudControllerHost extends BaseCrudController<T, C, U, F> {
    constructor(@Inject(service) crudService: ICrudService<T, C, U>, @Inject(CRUD_OPTIONS) options: any) {
