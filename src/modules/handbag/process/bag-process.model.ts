...
export const ErrBagProcessHasProduction = new Error(
  'Công đoạn đang được sử dụng trong sản xuất, không thể xóa',
);
export const ErrBagProcessHasPositions = new Error(
  'Công đoạn đang liên kết với vị trí công việc, không thể xóa',
);
...
export const bagProcessSchema = z.object({
  ...
  standardOutput: z.coerce
    ...
    .refine((n) => String(n).split('.')[1]?.length <= 2, {
      message:
        'Sản lượng tiêu chuẩn chỉ được phép có tối đa 2 chữ số thập phân',
    }),
